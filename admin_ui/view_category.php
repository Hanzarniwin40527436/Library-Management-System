    ...
    <main>
        <div class="container-fluid m-0 p-4">
                        ...
                        <div class="card">
                            <div class="card-body">
                                <form action="../actions/category_update.php" method="POST">
                                    <div class="input-group">
                                        <input type="text" name="category" class="form-control" placeholder="Update Category..." required value="<?php if(isset($one_row)){ echo $one_row->name;}?>">
            ...
            <table class="table table-bordered table-striped">
                ...
                <?php foreach ($pagination as $category) : ?>
                    <tr>
                        <td><?= ++$start ?></td>
                        <td><?= $category->name ?></td>
                        <td>
                            <div class="btn-group">
                                <a href="view_category.php?id=<?= $category->id?>" class="btn btn-outline-primary"><i class='bx bxs-edit'></i> Edit</a>
                                <a href="#" data-id="<?= $category->id ?>" class="btn btn-outline-danger"><i class='bx bxs-trash'></i> Delete</a>
                            </div>
                        </td>
                    </tr>
                <?php endforeach ?>
            ...
            <div class="row ">
                <div class="col d-flex justify-content-center">
                    <nav aria-label="Page navigation example">
                        <ul class="pagination">
                            <li class="page-item"><a class="page-link" href="view_category.php?page=<?= $previous ?>">Previous</a></li>
                            <?php for($i = 1; $i <= $pages; $i++):?>
                                <li class="page-item">
                                    <a class="page-link" href="view_category.php?page=<?= $i ?>"><?= $i ?></a>
                                </li>
                            <?php endfor; ?>
                            <li class="page-item"><a class="page-link" href="view_category.php?page=<?= $next ?>">Next</a></li>
                        </ul>
                    </nav>
                </div>
            </div>
        ...
        $(document).ready(function() {
            ...
            $('.btn-outline-danger').click(function(e) {
                ...
                bootbox.confirm('Do you really want to delete?', function(result){
                    if(result){
                        $.ajax({
                            url: '../actions/category_delete.php',
                            type: 'post',
                            data: {id: deleteid},
                            success: function(){
                                location.reload();
